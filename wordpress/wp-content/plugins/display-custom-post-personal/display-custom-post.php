<?php
/* Plugin Name: Display Custom Post Personal
      	...
      	if( $my_query->have_posts() ) : while( $my_query->have_posts() ) : $my_query->the_post();
			...
			if($length !== '' && $length !== NULL){
				...
				if (get_field('description')) {
					$dcp_strip_content = substr(get_field('description'), 0, 120) . '<span class="dots">...</span>';
				}else{
					$dcp_content = get_the_content($post->ID);
					$dcp_strip_content = substr($dcp_content, 0, 120) . '<span class="dots">...</span>';
				}
			}
			if (get_field('cover_art'))
			{
				$post_image = get_field('cover_art');
				echo get_field('cover_art');
			} else {
				...
				$html .= "<div class='dcp-post'>";
					$html .= "<div class='dcp-post-wrapper'>";
						$html .= "<div class='dcp-post-thumb'>";
							$html .= "<a href=". esc_url( get_permalink() ) . ">" . $post_image . "</a>";
						$html .= "</div>";
